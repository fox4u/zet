/*
 *  Zet SoC top level file for Colorlight i5 board
...
`ifdef SIMULATION
    output        spinor_clk_,
`endif
    output        spinor_cs_, /* synthesis syn_keep=1 */
    input	        spinor_so_,
    output        spinor_si_, /* synthesis syn_keep=1 */

    // sdram signals
    output [10:0] sdram_addr_,
    inout  [31:0] sdram_data_,
  ...
  wire [15:0] fl_dat_o_s;
  wire [15:0] fl_dat_i_s;
  wire        fl_tga_i_s;
  wire [19:1] fl_adr_i_s;
  wire [ 1:0] fl_sel_i_s;
  wire        fl_we_i_s;
  wire        fl_cyc_i_s;
  wire        fl_stb_i_s;
...
`ifndef SIMULATION
  ...
  always @(posedge vga_clk)
    {vga_rst, vga_rst_r} <= {vga_rst_r, rst_r};
`else
  wire rst;
  wire vga_rst;
  assign rst = !rst_lck;
...
/*  
  pll1 pll (
    .CLKI   (clk_25_),
    .CLKOP  (tmds_clk),   // 125 Mhz
    .CLKOS  (sdram_clk),  // 100 Mhz
    .CLKOS2 (clk),        // 12.5 Mhz
    .LOCK   (lock)
  );
*/
...
`endif
  ...
  wb_abrgr wb_spinorbrg (
    ...
    .wbm_clk_i (sdram_clk),
    .wbm_adr_o (fl_adr_i_s),
    .wbm_dat_o (fl_dat_i_s),
    .wbm_dat_i (fl_dat_o_s),
    .wbm_sel_o (fl_sel_i_s),
    .wbm_tga_o (fl_tga_i_s),
    .wbm_stb_o (fl_stb_i_s),
    .wbm_cyc_o (fl_cyc_i_s),
    .wbm_we_o  (fl_we_i_s),
    .wbm_ack_i (fl_ack_o_s)
  );

  wbspinor #(
    .word_addr_offset(23'he0000)
  ) u_flash_0 (
    .wb_clk_i(sdram_clk),
    .wb_rst_i(rst),
    .wb_dat_i(fl_dat_i_s),
...
`ifndef SIMULATION
  assign spiclk_en = ~spinor_cs_;
  USRMCLK spiclk (.USRMCLKI(spinor_clk), .USRMCLKTS(spiclk_en)); 
`else
  assign spinor_clk_ = spinor_clk;
`endif
  ...
  .m0_adr ({2'b01, vga_lcd_fml_adr}),  // 1 - 2 MB Addressable memory range
  ...
  .m1_adr ({2'b00, fml_fmlbrg_adr}),  // 0 - 1 MB Addressable memory range
  ...
  .m2_adr ({2'b01, vga_cpu_fml_adr}),  // 1 - 2 MB Addressable memory range
  ...
  .m3_adr ({2'b10, 20'b0}),  // 2 - 3 MB Addressable memory range
  .m3_stb (1'b0),
  .m3_we  (1'b0),
  .m3_ack (),
  .m3_sel (2'b00),
  ...
  .m4_adr ({2'b11, 20'b0}),  // 3 - 4 MB Addressable memory range
  .m4_stb (1'b0),
  .m4_we  (1'b0),
  .m4_ack (),
  .m4_sel (2'b00),
  ...
  .m5_adr ({3'b111, 19'b0}),  // 4 - 5 MB Addressable memory range
  .m5_stb (1'b0),
  .m5_we  (1'b0),
  .m5_ack (),
  .m5_sel (2'b00),
  ...
  */
  assign uart_dat_o = 16'h0;
  assign uart_ack_o = uart_stb_i & uart_cyc_i;
  assign intv[4] = 1'b0; 
...
/*  
  dvi_encoder dvi_encoder1(
    ...
    .pixelclk5x  (tmds_clk          ),
    .rst_p       (rst               ),
    .blue_din    ({tft_lcd_b_, 4'b0}),
    .green_din   ({tft_lcd_g_, 4'b0}),
    .red_din     ({tft_lcd_r_, 4'b0}),
    .hsync       (tft_lcd_hsync_    ),
    .vsync       (tft_lcd_vsync_    ),
    .de          (tft_lcd_de_       ),
    .tmds_clk_p  (gpdi_dp[3]        ),
    .tmds_clk_n  ( ),
    .tmds_data_p (gpdi_dp[2:0]      ),
    .tmds_data_n ( )
  );  
*/
