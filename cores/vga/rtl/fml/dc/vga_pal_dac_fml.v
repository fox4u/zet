...
module vga_pal_dac_fml (
  ...
  always @(posedge vga_clk)
    index_gm <= vga_rst ? 8'h0 : character_pal_dac_i;        

  // Horiz sync
  always @(posedge vga_clk)
    { horiz_sync, horiz_sync_p } <= vga_rst ? 3'b0 : { horiz_sync_p[1:0], horiz_sync_pal_dac_i };

  // Video_on pipe
  always @(posedge vga_clk)
    video_on_h_p <= vga_rst ? 2'b0 : { video_on_h_p[0], video_on_h_pal_dac_i };
